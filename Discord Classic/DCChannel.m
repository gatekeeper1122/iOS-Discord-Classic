...
-(void)checkIfRead{
	NSString* readStateLastMessageId = [DCServerCommunicator.sharedInstance.readStates valueForKey:self.snowflake];
	if([readStateLastMessageId isEqualToString:self.lastMessageId])
		[self setRead:true];
	else
		[self setRead:false];
